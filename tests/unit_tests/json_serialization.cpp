...
namespace
{
    ...
    make_spinner_transaction(cryptonote::account_public_address const& to)
    {
        cryptonote::transaction tx{};
        if (!cryptonote::construct_spinner_tx(0, 0, 5000, 500, 500, to, tx))
...
TEST(JsonSerialization, SpinnerTransaction)
{
    cryptonote::account_base acct;
    acct.generate();
    const auto spinner_tx = make_spinner_transaction(acct.get_keys().m_account_address);

    crypto::hash tx_hash{};
    ASSERT_TRUE(cryptonote::get_transaction_hash(spinner_tx, tx_hash));

    cryptonote::transaction spinner_tx_copy = test_json(spinner_tx);

    crypto::hash tx_copy_hash{};
    ASSERT_TRUE(cryptonote::get_transaction_hash(spinner_tx_copy, tx_copy_hash));
    EXPECT_EQ(tx_hash, tx_copy_hash);

    cryptonote::blobdata tx_bytes{};
    cryptonote::blobdata tx_copy_bytes{};

    ASSERT_TRUE(cryptonote::t_serializable_object_to_blob(spinner_tx, tx_bytes));
    ASSERT_TRUE(cryptonote::t_serializable_object_to_blob(spinner_tx_copy, tx_copy_bytes));
...
TEST(JsonSerialization, RegularTransaction)
{
    ...
    const auto spinner_tx = make_spinner_transaction(acct1.get_keys().m_account_address);
    const auto tx = make_transaction(
        acct1.get_keys(), {spinner_tx}, {acct2.get_keys().m_account_address}, false, false
    );
...
TEST(JsonSerialization, RingctTransaction)
{
    ...
    const auto spinner_tx = make_spinner_transaction(acct1.get_keys().m_account_address);
    const auto tx = make_transaction(
        acct1.get_keys(), {spinner_tx}, {acct2.get_keys().m_account_address}, true, false
    );
...
TEST(JsonSerialization, BulletproofTransaction)
{
    ...
    const auto spinner_tx = make_spinner_transaction(acct1.get_keys().m_account_address);
    const auto tx = make_transaction(
        acct1.get_keys(), {spinner_tx}, {acct2.get_keys().m_account_address}, true, true
    );
