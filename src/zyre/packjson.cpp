...
#define MONERO_DEFAULT_LOG_CATEGORY "zyre.packjson"
  ...
  bool pack2json(const std::string& data, std::size_t& off, std::string& out)
  {
    struct visitor
    {
      ...
      void parse_error(size_t parsed_offset, size_t error_offset) {
        MLOG_RED(el::Level::Error, "msgpack parse error, parsed offset: " << parsed_offset << ", error offset: " << error_offset);
      }
      void insufficient_bytes(size_t parsed_offset, size_t error_offset) {
        MLOG_RED(el::Level::Error, "msgpack insufficient bytes, parsed offset: " << parsed_offset << ", error offset: " << error_offset);
      }
